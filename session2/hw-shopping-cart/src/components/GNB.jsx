...
// import ArrowBackIcon from '/images/svg/arrow-back.svg'; // 절대 경로로 수정
...
export const GNB = ({ type, pageName, onClick, ...rest }) => {
    ...
    }, [router.asPath, type]); // useLocation 대신 router.asPath 사용

    const handleClickBack = () => {
        router.back(); // navigate(-1) 대신 router.back() 사용
    };
                    ...
                    <NavBar>
                        {GNB_MENU.map((menu) => (
                            <NavButton
                                ...
                                onClick={() => router.push(menu.path)} // navigate 대신 router.push 사용
                            >
                                {menu.name}
                            </NavButton>
                        ))}
